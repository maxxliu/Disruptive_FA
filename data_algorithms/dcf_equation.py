...
def partial(func, *args, **keywords): #this is a curry function that will be used to get y value from the linear regression line
    ...
    and x value to get its corresponding y value 
...
def get_current_ebit(fin_dict):
    '''
    This helper function finds a company's 
    current Earnings before interest and tax.
   EBIT =  total revenue - cost of revenue 
   - sga (sales, general, and admin) - depreciation
   This function accesses the financial dictionary 
   for all these values.

   Inputs: a Financial Dictionary

   Outputs: A company's EBIT
    '''
...
def get_rd(fin_dict):
    '''
    This function finds a company's cost of debt.  
    It takes a financial dictionary as its input.
    ...
    the first index value is AAA, the next AA,
    the next A etc.  There are two tables, 
    one for companies above 5B market cap and ones below 5B.  
    We find a company'smarket cap in the financial dictionary. 

    Inputs: A financial dictionary

    Outputs: a Company's cost of debt
    '''
...
def linear_regression(year_value_lst):
    '''
    Takes in a list of financial values for a set 
    number of years and returns the slope and y intercept 
    of its respective linear regression line.
    This linear_regression function also accounts for missing values.  
    If it sees more than one zero but total zeros are less than 
    total number of values, it sets potential_error as True.

    Inputs: list of financial values

    Outputs: slope, y intercept, potential error
    '''
...
def get_values(slope, y_int, year):
    ...
    Inputs: slope, y intercept, and year
...
def WACC(fin_dict, expected_return):
    ...
    This function takes in financial dictionary and 
    expected return, which is an investor's risk averse score.  
    WACC = (equity/total equity and debt) * cost_equity + 
    (current_debt / total equity and debt) * rd * (1-tax_rate).
    The function returns a company's WACC value, 
    which will be used to discount future cash.

    Inputs: financial dictionary and expected return

    Outputs: a company's WACC value
    '''
    risk_free_rate = 0.02587 #10 year U.S. Treasury Bond
    tax_rate = 0.35
    current_debt = fin_dict['Balance Sheet']['Long-Term Debt'][-1] + \
    fin_dict['Balance Sheet']\
    ...
    WACC_val = (current_equity/total_d_e) * cost_equity + \
    (current_debt/ total_d_e) * rd* (1 - tax_rate) #rd not pulled yet
    return WACC_val

def NWC(fin_dict):
    '''
    Accesses a financial dictionary for totat assets 
    and total Liabilities for a given year.  
    It takes the x_difference, which the 
    net working capital and appends nwc to a nwc list
    The function returns the nwc

    Inputs: Financial Dictionary

    Outputs: A list of net working capital
    '''
    nwc_list = []
    for i in range(len(fin_dict['Balance Sheet']['Total Current Assets'])):
        nwc_value = fin_dict['Balance Sheet']['Total Current Assets'][i] \
        - fin_dict['Balance Sheet']['Total Current Liabilities'][i]
        nwc_list.append(nwc_value)
    return nwc_list

def dcf_feasibility(fin_dict):
    '''
    This helper function tests to see if there are less than 
    3 years worth of data, no data, and if revenue = 0. 
    If either are True, the function returns True, 
    meaning that there is not sufficient data to create a DCF. 

    Inputs: Financial Dictionary

    Outputs: A true value in data not DCF feasible
    '''
...
def find_slope_intercept(fin_dict):
    '''
    This function finds all the slope and y intercepts 
    for a financial value's respective linear regression line.  
    This function returns a list of tuples where each tuple is the 
    corresponding curry partial function and potential error value.  
    It also returns the current nwc

    Inputs: A financial Dictionary

    Outputs: A list of partial functions and past nwc
    '''
    curry_error_lst = []

    s, y, p_error = \
    linear_regression(fin_dict['Income Statement']['Total Revenue'])
    curry = partial(get_values, slope = s, y_int = y)
    curry_error_lst.append((curry, p_error))

    s, y, p_error = \
    linear_regression(fin_dict['Income Statement']['Cost of Revenue'])
    curry = partial(get_values, slope = s, y_int = y)
    curry_error_lst.append((curry, p_error))

    s, y, p_error = \
    linear_regression(fin_dict['Income Statement']\
        ['Sales, General and Admin.'])
    curry = partial(get_values, slope = s, y_int = y)
    curry_error_lst.append((curry, p_error))

    s, y, p_error = \
    linear_regression(fin_dict['Cash Flow']['Depreciation'])
    curry = partial(get_values, slope = s, y_int = y)
    curry_error_lst.append((curry, p_error))
    ...
    s, y, p_error = \
    linear_regression(fin_dict['Cash Flow']['Capital Expenditures'])
    curry = partial(get_values, slope = s, y_int = y)
    curry_error_lst.append((curry, p_error))
...
def get_financial_values(future_year, curry_error_lst, tax_rate, past_nwc):
    '''
    This function takes in the future year, the list of slopes and y int, 
    the tax rate, and the past nwc (will be used to calculate delta nwc).  
    It saves total revenue, cost of revenue, gross profit, sga,
    ebitda, depreciation, ebit, nwc, delta_nwc, capital expenditures, 
    and fcf for a given future year to financal value list.
    This function finally returns this financial 
    value list and the future year's nwc. 

    Inputs: future year, curry list, tax rate, past nwc

    Outputs: A list of financial values and current nwc
    '''
    financial_val_lst = []
    ...
    financial_val_lst += [total_revenue] + [cost_of_revenue] + \
    [gross_profit] + [sga] + [ebitda] + [depreciation] + \
    [ebit] + [delta_nwc] + [capex] + [fcf] 
    return financial_val_lst, nwc

#These are index labels for the fcf breakdown dataframe
financial_term_lst = ['Total Revenue', 'Cost of Revenue', \
'Gross Profit', 'SGA', 'EBITDA', 'Depreciation',
...
def dcf_calculator(fin_dict, expected_return, growth_rate=0.0124):
    '''
    Takes a nested dictionary, expected_return \
    (based on an investor's risk averse score), 
    growth_rate (default is average between interest rate and GDP growth) \
    and returns actual price per share of company
    the financial calcuations table for all future cash flows, \
    list of years, inaccurate rating, and buy/ sell rating.
    The calculator assumes the tax rate to be 35 percent\
     and that preferred stock is negligible
    '''
    tax_rate = 0.35
    pvfcf = 0
    ...
    for year in range(1,6): #each year represents one of the 5 future years
        ...
        financial_val_lst, past_nwc = \
        get_financial_values(future_year, slope_int_lst, tax_rate, past_nwc)
    ...
    number_of_shares = fin_dict['Summary Data']['market cap']\
     / fin_dict['Summary Data']['previous close']
