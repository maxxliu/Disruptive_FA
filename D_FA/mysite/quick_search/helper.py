...
def create_dict(stock_ticker):
    ...
    sd = Summary_Data.objects.get(ticker=s)
    dd = Data_Date.objects.get(ticker=s)

    fs_dict = {}
    for data in fs:
        s_type = data.statement_type
        l_item = data.line_item
        if s_type not in fs_dict:
            fs_dict[s_type] = {}
        fs_dict[s_type][l_item] = [data.year_1_val, data.year_2_val,\
                                    data.year_3_val, data.year_4_val]

    fs_dict['Dates'] = [dd.year_1, dd.year_2, dd.year_3, dd.year_4]

    fs_dict['Summary Data'] = {}
    fs_dict['Summary Data']['target'] = float(sd.target)
    fs_dict['Summary Data']['year high'] = float(sd.year_high)
    fs_dict['Summary Data']['year low'] = float(sd.year_low)
    fs_dict['Summary Data']['beta'] = float(sd.beta)
    fs_dict['Summary Data']['market cap'] = float(sd.market_cap)
    fs_dict['Summary Data']['previous close'] = float(sd.previous_close)
